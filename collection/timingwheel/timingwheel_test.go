...
func TestNewTimingWheel(t *testing.T) {
	_, err := timingwheel.NewTimingWheel(0, 10, func(key, value any) {})
	assert.NotNil(t, err)
}

func TestTimingWheel_Drain(t *testing.T) {
	ticker := timex.NewFakeTicker()
	tw, _ := timingwheel.NewTimingWheelWithTicker(testStep, 10, func(k, v any) {
	}, ticker)
	...
	tw.Stop()
	assert.Equal(t, timingwheel.ErrClosed, tw.Drain(func(key, value any) {}))
}
...
func TestTimingWheel_SetTimerWrongDelay(t *testing.T) {
	ticker := timex.NewFakeTicker()
	tw, _ := timingwheel.NewTimingWheelWithTicker(testStep, 10, func(k, v any) {}, ticker)
	defer tw.Stop()
...
func TestTimingWheel_SetTimerAfterClose(t *testing.T) {
	...
	tw, _ := timingwheel.NewTimingWheelWithTicker(testStep, 10, func(k, v any) {}, ticker)
	tw.Stop()
	assert.Equal(t, timingwheel.ErrClosed, tw.SetTimer("any", 3, testStep))
}

func TestTimingWheel_MoveTimer(t *testing.T) {
	run := syncx.NewAtomicBool()
	ticker := timex.NewFakeTicker()
	tw, _ := timingwheel.NewTimingWheelWithTicker(testStep, 3, func(k, v any) {
		assert.True(t, run.CompareAndSwap(false, true))
		assert.Equal(t, "any", k)
		assert.Equal(t, 3, v.(int))
		ticker.Done()
	...
	assert.True(t, run.True())
	tw.Stop()
	assert.Equal(t, timingwheel.ErrClosed, tw.MoveTimer("any", time.Millisecond))
}
...
func TestTimingWheel_RemoveTimer(t *testing.T) {
	...
	tw.Stop()
	assert.Equal(t, timingwheel.ErrClosed, tw.RemoveTimer("any"))
}
...
func TestTimingWheel_SetTimer(t *testing.T) {
	...
	for _, test := range tests {
		test := test
		t.Run(strconv.Itoa(test.slots), func(t *testing.T) {
			t.Parallel()
			...
			tw, err := timingwheel.NewTimingWheelWithTicker(testStep, test.slots, func(key, value any) {
...
func TestTimingWheel_SetAndMoveThenStart(t *testing.T) {
	...
	for _, test := range tests {
		test := test
		t.Run(string(mathx.RandStr(4)), func(t *testing.T) {
			t.Parallel()
...
func BenchmarkTimingWheel(b *testing.B) {
	...
	tw, _ := timingwheel.NewTimingWheel(time.Second, 100, func(k, v any) {})
	for i := 0; i < b.N; i++ {
